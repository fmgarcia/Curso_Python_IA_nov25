...
"""
XGBoost Classifier para detección de tumores
...
MODEL_PATH = os.path.join(os.path.dirname(__file__), '..', 'weights', 'xgboost.joblib')
FEATURE_NAMES_PATH = os.path.join(os.path.dirname(__file__), '..', 'weights', 'feature_names.joblib')
ACCURACY_PATH = os.path.join(os.path.dirname(__file__), '..', 'weights', 'xgboost_accuracy.json')
...
def train():
    """
    Entrena el modelo XGBoost y guarda los pesos
    Returns:
    ...
    return {
        ...
        "message": f"Modelo XGBoost entrenado y guardado correctamente{dataset_msg}"
    }
...
def predict(features: dict):
    ...
    if not os.path.exists(MODEL_PATH):
        raise FileNotFoundError("El modelo no ha sido entrenado. Ejecute /train/xgboost primero.")
