...
"""
Voting Classifier para detección de tumores
Integra DecisionTreeClassifier, LogisticRegression y KNeighborsClassifier
...
MODEL_PATH = os.path.join(os.path.dirname(__file__), '..', 'weights', 'voting.joblib')
FEATURE_NAMES_PATH = os.path.join(os.path.dirname(__file__), '..', 'weights', 'feature_names.joblib')
ACCURACY_PATH = os.path.join(os.path.dirname(__file__), '..', 'weights', 'voting_accuracy.json')
...
def train():
    """
    Entrena el modelo Voting Classifier y guarda los pesos
    Returns:
    ...
    return {
        ...
        "message": f"Modelo Voting Classifier entrenado y guardado correctamente{dataset_msg}"
    }
...
def predict(features: dict):
    ...
    if not os.path.exists(MODEL_PATH):
        raise FileNotFoundError("El modelo no ha sido entrenado. Ejecute /train/voting primero.")
